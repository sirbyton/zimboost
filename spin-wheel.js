(function () {
  ...
  function findWheelAnchor() {
    ...
    var candidates = all.filter(function (el) {
      ...
      return phraseVariants.some(function (p) { return t.includes(p); }) && isVisible(el);
    });
  ...
  function tryInit() {
    if (document.getElementById("zimboost-real-wheel")) return true;
    var anchor = findWheelAnchor();
    if (!anchor) return false;
    var container = anchor;
    var levels = 0;
    while (container.parentElement && levels < 3) {
